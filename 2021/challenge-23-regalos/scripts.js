function canReconfigure(from, to) {
  ...
  console.log("Arrays", fromArray, toArray);
  if (fromArray.length !== toArray.length) response = false;

  console.log("response", response);
  return response;
}

canReconfigure("AA", "MID");
...
no se puede asignar al mismo carácter a dos letras distintas (pero sí a si mismo) y, claro,
la longitud del string debe ser el mismo.
Necesitamos una función que nos diga si podemos reconfigurar una máquina para que de un regalo
pueda pasar a fabricar otro según las reglas mencionadas. Lo mejor es que veamos un ejemplo:
*/
...
/* no se puede hacer la transformación:
X -> X
B -> X (FALLO, no mantiene el orden de transformación y la B no puede asignarse a la X que ya se asignó 
        a otra) 
O -> B
